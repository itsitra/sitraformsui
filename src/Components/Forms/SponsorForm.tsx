...
export default function SponsorForm({ id, data, personaldetails, contactpersondetails, otherdetails, changePersonalData, changeContactPersonDetails, changeOtherDetails, handleSubmit, paymentDetails, changePaymentDetails, gstStatus, updateGstStatus, lists, setLists }: SponsorFormTypes) {
     ...
     const closeModal = () => {
          ...
          changeOtherDetails({ ...otherdetails, sponsorpackage: sessionStorage.getItem('eventcost') })
     }
     const saveAndOpenPaymentModal = (e: React.MouseEvent<HTMLButtonElement>) => {
          e.preventDefault()
          handleSubmit('sponsor', e, setPaymentModal)
          // setPaymentModal(true)

     }

     const setSponsorPAckage = (e: React.ChangeEvent<HTMLInputElement>, type: string) => {
          sessionStorage.setItem("eventcost", e.target.value.replace(/\,/g, ''))
          changeOtherDetails({ ...otherdetails, sponsorpackage: Number(e.target.value.replace(/\,/g, '')), sponsortype: type === 'platinum' ? 'platinum' : type === 'diamond' ? 'diamond' : type === 'gold' ? 'gold' : 'silver' })
     }
     ...
     const changeAmount = (e: number, amount: number) => {
          ...
          changeOtherDetails({ ...otherdetails, sponsorpackage: calAmount })
     }

     useEffect(() => {
          document.title = 'Sponsor Form';
          if (sessionStorage.getItem('eventcost')) {
               changeOtherDetails({ ...otherdetails, sponsorpackage: Number(sessionStorage.getItem("eventcost")) })
          }
     }, [])
     return (
          <>
               {data.length > 0 ? <div className="h-screen m-auto flex items-center justify-center md:w-1/2 lg:w-1/3">
                    <div id="form" className="w-full h-full m-5  py-5 space-y-2 relative">
                         ...
                         <form id="form" className="flex flex-col gap-4">
                              ...
                              <FormCard title="Sponsorship Offerings">
                                   <div className="p-5 space-y-4">
                                        <div className="form-control w-full ">
                                             <label className="label mb-2">
                                                  <span className="label-text font-bold">Sponsorship Offering<strong className="text-red-500">*</strong></span>
                                                  <span className="label-text-alt">[Note: Including GST] </span>
                                                  <a href={data[0].eventData.eventbrochure} className="link link-secondary" download target="_blank">Sponsorship info</a>
                                             </label>
                                             <hr />
                                             <div className="flex gap-5 flex-col my-3">
                                                  <label className="label cursor-pointer space-x-3">
                                                       <div className="flex flex-col gap-2">
                                                            ...
                                                            <span className="label-text">&#8377;{Number(eventFormData!.platinum).toLocaleString()}/</span>
                                                       </div>
                                                       <input type="radio" name="radio-6" value={eventFormData!.platinum} className="radio checked:bg-blue-500" onChange={(e) => { setSponsorPAckage(e, 'platinum') }} />
                                                  </label>
                                                  <label className="label cursor-pointer space-x-3">
                                                       <div className="flex flex-col gap-2">
                                                            <span className="label-text">Diamond</span>
                                                            <span className="label-text">&#8377;{Number(eventFormData!.diamond).toLocaleString()}/</span>
                                                       </div>
                                                       <input type="radio" name="radio-6" value={eventFormData!.diamond} className="radio checked:bg-blue-500" onChange={(e) => { setSponsorPAckage(e, 'diamond') }} />
                                                  </label>
                                                  <label className="label cursor-pointer space-x-3">
                                                       <div className="flex flex-col gap-2">
                                                            <span className="label-text">Gold</span>
                                                            <span className="label-text">&#8377;{Number(eventFormData!.gold).toLocaleString()}/</span>
                                                       </div>
                                                       <input type="radio" name="radio-6" className="radio checked:bg-blue-500" value={eventFormData!.gold} onChange={(e) => { setSponsorPAckage(e, 'gold') }} />
                                                  </label><label className="label cursor-pointer space-x-3">
                                                       <div className="flex flex-col gap-2">
                                                            <span className="label-text">Silver</span>
                                                            <span className="label-text">&#8377;{Number(eventFormData!.silver).toLocaleString()}/</span>
                                                       </div>
                                                       <input type="radio" name="radio-6" className="radio checked:bg-blue-500" value={eventFormData!.silver} onChange={(e) => { setSponsorPAckage(e, 'silver') }} />
                                                  </label>
                                             </div>
                                             <hr />
                                        </div>
                    ...
                    <div className="lg:hidden">
                         <TotalCostMobile cost={0} />
                    </div>
                    {paymentModal && <PaymentCheck closeModal={closeModal} paymentDetails={paymentDetails} changePaymentDetails={changePaymentDetails} formtype={'S'} amount={otherdetails.sponsorpackage} custname={personaldetails.name} custemail={personaldetails.email} custmobile={personaldetails.mobileno} changeAmount={changeAmount} lists={lists} setLists={setLists} generalDetails={personaldetails} />}
               </div>
                    : <Loader />
               }
